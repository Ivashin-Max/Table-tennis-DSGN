...
import { openModal, setLoading, setEmptyData, setData } from "../../store/reducer";
import { ILink, ILinksAdd, ITournamentAdd, ITournamentPatch, IParticipantGet } from "../../types/fetch";
...
export const getParticipantsLikeAdmin = (tournamentId: number) => async (dispatch: any, getState: any) => {
  const userJWT = getUser().jwt;
  if (tournamentId === 0) {
    ...
    try {
      ...
      const apiUrl = url.back + url.endpoints.admin.getParticipantsAdminLike + tournamentId;
