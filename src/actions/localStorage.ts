...
export const addLocalStorageItem = (key: string, newValue: string) => {
  ...
  if (existingValues) {
    if (existingValues.split("$").includes(newValue))
      console.log(`В локалсторадж уже хранится ${newValue} по ключу ${key}`);
...
export const setDefaultCity = (cityId: string) => {
  return reactLocalStorage.setObject("defaultCity", cityId);
};

export const getDefaultCity = (): string | undefined => {
  return reactLocalStorage.getObject("defaultCity");
};
